...
import { formatDate, calculateFollowUpDate } from '../utils/formatDate';
    ...
    <div className="page-container" style={{ maxWidth: '100%' }}>
      ...
      <div style={{
        ...
        <div className="glass-panel" style={{ padding: '0.85rem', textAlign: 'center' }}>
          <div style={{ fontSize: '1.4rem', fontWeight: 800, color: '#FFF' }}>{stats.total}</div>
          <div style={{ fontSize: '0.72rem', color: 'var(--text-muted)' }}>Total Démarches</div>
        </div>
        <div className="glass-panel" style={{ padding: '0.85rem', textAlign: 'center' }}>
          <div style={{ fontSize: '1.4rem', fontWeight: 800, color: 'var(--status-ready)' }}>{stats.pending}</div>
          <div style={{ fontSize: '0.72rem', color: 'var(--text-muted)' }}>En Préparation</div>
        </div>
        <div className="glass-panel" style={{ padding: '0.85rem', textAlign: 'center' }}>
          <div style={{ fontSize: '1.4rem', fontWeight: 800, color: 'var(--status-sent)' }}>{stats.sent}</div>
          <div style={{ fontSize: '0.72rem', color: 'var(--text-muted)' }}>Envoyées</div>
        </div>
        <div className="glass-panel" style={{ padding: '0.85rem', textAlign: 'center' }}>
          <div style={{ fontSize: '1.4rem', fontWeight: 800, color: 'var(--status-followup)' }}>{stats.followUp}</div>
          <div style={{ fontSize: '0.72rem', color: 'var(--text-muted)' }}>Relance Requise</div>
        </div>
        <div className="glass-panel" style={{ padding: '0.85rem', textAlign: 'center' }}>
          <div style={{ fontSize: '1.4rem', fontWeight: 800, color: 'var(--status-interview)' }}>{stats.interview}</div>
          <div style={{ fontSize: '0.72rem', color: 'var(--text-muted)' }}>Entretiens</div>
        </div>
        <div className="glass-panel" style={{ padding: '0.85rem', textAlign: 'center' }}>
          <div style={{ fontSize: '1.4rem', fontWeight: 800, color: 'var(--status-accepted)' }}>{stats.accepted}</div>
          <div style={{ fontSize: '0.72rem', color: 'var(--text-muted)' }}>Offres Reçues</div>
        </div>
        <div className="glass-panel" style={{ padding: '0.85rem', textAlign: 'center' }}>
          <div style={{ fontSize: '1.4rem', fontWeight: 800, color: 'var(--status-rejected)' }}>{stats.rejected}</div>
          <div style={{ fontSize: '0.72rem', color: 'var(--text-muted)' }}>Refusées</div>
        </div>
      </div>
          ...
          >
            ...
            <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: '1.5rem' }}>
              <div>
                ...
                <h3 style={{ fontSize: '1.35rem', color: '#FFF' }}>{selectedApp.jobTitle}</h3>
                <div style={{ fontSize: '0.88rem', color: 'var(--color-gold-light)', fontWeight: 600, marginTop: '0.2rem' }}>
            ...
            <div style={{ marginBottom: '1.5rem' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.4rem' }}>
                ...
                <button type="button" onClick={handleSaveNotes} className="btn btn-gold-outline btn-sm" style={{ padding: '0.2rem 0.6rem', fontSize: '0.72rem' }}>
                  Sauvegarder notes
                </button>
              </div>
              <textarea
                ...
                placeholder="Ex: Entretien technique passé avec le Tech Lead. Retour attendu sous 5 jours..."
            ...
            {/* Modal Actions */}
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '1rem', borderTop: '1px solid var(--border-subtle)', paddingTop: '1.25rem' }}>
              <button
                type="button"
